...
import cn.dbsec.core.shiro.CustomShiroSessionDAO;
import cn.dbsec.user.bo.UserOnlineBo;
import cn.dbsec.util.tools.IPv4Util;
	...
	private UserOnlineBo getSessionBo(Session session) {
		...
		if (obj instanceof SimplePrincipalCollection) {
			...
			if (null != obj && obj instanceof UUser) {
				...
				// 主机的ip地址
				userBo.setHost(IPv4Util.getRealIp(session.getHost()));
				// session ID
