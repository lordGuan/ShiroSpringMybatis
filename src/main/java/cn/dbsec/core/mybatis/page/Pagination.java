package cn.dbsec.core.mybatis.page;
...
public class Pagination<T> extends SimplePage implements java.io.Serializable, Paginable {
	...
	public String getSiAjaxPageHtml() {
		...
		if (this.getPageNo() > 1) {
			...
			pageHtml.append("<li><a href='javascript:;'  onclick='goPageByAjax(" + (this.getPageNo() - 1)
					+ ")'>上一页</a></li>");
		}
		for (int i = (this.getPageNo() - 2 <= 0 ? 1 : this.getPageNo() - 2), no = 1; i <= this.getTotalPage() && no < 6; i++, no++) {
			if (this.getPageNo() == i) {
	...
	public String getPageHtml() {
		...
		if (this.getPageNo() > 1) {
			...
			pageHtml.append("<li><a href='javascript:;'  onclick='_submitform(" + (this.getPageNo() - 1)
					+ ")'>上一页</a></li>");
		}
		for (int i = (this.getPageNo() - 2 <= 0 ? 1 : this.getPageNo() - 2), no = 1; i <= this.getTotalPage() && no < 6; i++, no++) {
			if (this.getPageNo() == i) {
